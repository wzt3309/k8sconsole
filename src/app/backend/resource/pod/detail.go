...
func GetPodDetail(client kubernetes.Interface, namespace, name string) (*PodDetail, error) {
	glog.Infof("Getting details of %s pod in %s namespace", name, namespace)

	channels := &common.ResourceChannels{
		ConfigMapList: common.GetConfigMapListChannel(client, common.NewOneNamespaceQuery(namespace), 1),
		SecretList: common.GetSecretListChannel(client, common.NewOneNamespaceQuery(namespace), 1),
	}
	...
	ctrl, err := getPodController(client, common.NewOneNamespaceQuery(namespace), pod)
	...
	podDetail := toPodDetail(pod, configMapList, secretList, ctrl,
...
func extractContainerInfo(containerList []v1.Container, pod *v1.Pod, configMaps *v1.ConfigMapList,
	secrets *v1.SecretList) []Container {
	...
	for _, container := range containerList {
		...
		for _, envVar := range container.Env {
			...
			if variable.ValueFrom != nil {
				variable.Value = evalValueFrom(variable.ValueFrom, &container, pod, configMaps, secrets)
			}
...
func evalValueFrom(src *v1.EnvVarSource, container *v1.Container, pod *v1.Pod, configMaps *v1.ConfigMapList,
